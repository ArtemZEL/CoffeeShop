    ...
    public class CoffeShopController : Controller
    {
        ...
        public IActionResult Card(string category = "Brazilian", string name = "Name")
        {
            var model = new ProductCardViewModel
            {
                ...
                OrderTerms = new List<string>
                {
                    "Заказы обрабатываются по будним дням с 10.00 до 18.00.",
                    "Заказ, поступивший в выходные дни, будет обработан утром понедельника.",
                    "Заказ, оформленный до 14.00, с понедельника по пятницу, может быть доставлен в этот же день после оформления."
                },
